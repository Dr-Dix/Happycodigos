...
private String estacao;
    ...
    public void inserir (){
         String sql = "INSERT INTO TbGerenciamentodeSafras(nomeAlimento, tempoColheita, epocaCultivo, qtdPlantou, qtdColheu, estacao)" + "VALUES(?,?,?,?,?,?)";
...
public void atualizar () throws SQLException {
            ConnectionFactory factory = new ConnectionFactory();
            String sql = "UPDATE TbGerenciamentodeSafras SET nomeAlimento = ?, tempoColheita = ?, epocaCultivo = ?, qtdPlantou = ?, qtdColheu = ? , estacao = ? WHERE codSafra = ?";        
            try (Connection c = factory.obtemConexao();                
            PreparedStatement ps = c.prepareStatement(sql)){            
                ps.setString(1, nomeAlimento); 
                    ps.setString(2, tempoColheita);
